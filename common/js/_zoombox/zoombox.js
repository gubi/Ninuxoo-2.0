...
zoombox = {
	 ...
	 * 
	 * **********************************/
	Dwidth : 		800, 		// Default width
	Dheight: 		600,		// Default height
	...
	maskOpacity:	0.9,		// Opacity of the black overlay,
	FLVPlayer:	"common/media/swf/FLVplayer.swf?params.scale=showall&params.menu=false;",   // URL of the FLV Player
	MP3Player:	"common/media/swf/mp3_player.swf",  // URL of the MP3 Player
	allowOverflow:	false,	// Automatically adjust width/height of the box if the content is larger than the window
	bindable:		"*[rel^='zoombox']",  // (jQuery expression) elements which have the zoombox behaviour
				...
				<div id="zoombox_contener">\
					<div id="zoombox_relative">\
						...
						<div id="zoombox_nav">\
							<table>\
								<tr>\
									<td width="39"><div id="zoombox_prev"></div><div id="zoombox_prev2"></div></td>\
	 ...
	 * 
	 * **********************************/
			// Modifica ai forms all'interno di un contenuto
	...
	open : function(url,width,height){
		...
		if(!$("#zoombox").length){
			...
			$("#zoombox_aplat").click(zoombox.close);		// We bind the "close" event
	 ...
	 * */
	close : function(){
		if (zoombox.formModificato){
			if (confirm("\u00C8 stato fatto click in un punto dello schermo che richiama la chiusura dell'attuale riquadro.\nChiudendolo si abbandoneranno tutte le modifiche in corso.\nSicuri di voler continuare?")){
	...
	resize : function(){
		// The new CSS for the box, final position
		if (document.getElementById("zoombox_contener") != null){
				var topPos = (zoombox.windowH() - zoombox.height - zoombox.margesV)/2 + zoombox.scrollY();
				var leftPos = (zoombox.windowW() - zoombox.width - zoombox.margesH)/2;
		...
		// Do we fix the box ?
		if(zoombox.fixed == true){
			$("#zoombox_contener").css("top",topPos);
		}
	},
	...
	next : function(){
		if (zoombox.formModificato){
			if (confirm("\u00C8 stato fatto click in un punto dello schermo che richiama un altro contenuto nell'attuale riquadro.\nConfermando si abbandoneranno tutte le modifiche in corso.\nSicuri di voler continuare?")){
	...
	getContent : function(url){
			...
			if(zoombox.filtreImg.test(url)){
				...
				content='<iframe src="'+url+'" width="100%" height="100%" border="0"></iframe>';
			}
			return content;
	},
