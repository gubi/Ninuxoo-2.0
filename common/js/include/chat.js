$.chat_me = function(id, username) {
	$.ajax({
		...
		type: "POST",
		data: {
			jCryption: $.jCryption.encrypt("id=" + id + "&username=" + username, password),
			type: "set_socket"
		},
...
$.check_connected = function() {
	...
	$.ajax({
		...
		type: "POST",
		data: {
			jCryption: $.jCryption.encrypt("current=" + $("#user_id").text(), password),
...
$(document).ready(function() {
	...
	$(".chat_status").click(function() {
		...
		$.ajax({
			...
			type: "POST",
			data: {
				jCryption: $.jCryption.encrypt("username=" + $("#user_email").text() + "&status=" + status, password),
