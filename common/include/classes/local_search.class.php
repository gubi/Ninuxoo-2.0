...
class local_search {
	...
	private function explodeTree($array, $delimiter = "_", $baseval = false) {
		...
		foreach ($array as $key => $val) {
			...
			if (empty($parentArr[$leafPart])) {
				...
				if(!$parsed["extension"]){
					if(str_replace($this->params["replace_uri"], "", $val) !== stripslashes(str_replace("\ ", " ", $this->params["path"]))){
						$parentArr["resources"][$k]["filetype"] = "DIRECTORY";
						$parentArr["resources"][$k]["uri"] = str_replace($this->params["replace_uri"], "", $val);
						$parentArr["resources"][$k]["filename"] = substr(strrchr($val, "/"), 1);
					}
				} else {
		...
		foreach($arr as $k => $v){
			...
			if(is_array($v) && $k != "resources"){
				...
				if($indent > 0){
					$plot[$f]["rank"] = $rank;
					$plot[$f]["label"] = $k;
					if(!is_array($this->plotTree($v, ($indent+1), false))) {
						$plot[$f]["children"] = array();
					} else {
						$plot[$f]["children"] = $this->plotTree($v, ($indent+1), false);
					}
					...
					$plot["rank"] = $rank;
					$plot["label"] = $k;
					$plot["children"] = $this->plotTree($v, ($indent+1), false);
	...
	private function browse(){
		...
		if(isset($this->params["path"]) && trim($this->params["path"]) !== "" && trim($this->params["path"]) !== "/"){
			...
			foreach($list as $k => $v){
				if($_get == "" || $_get == "/") {
					if(!in_array($v, $this->conf["NAS"]["smb_shares"])) {
						unset($list[$k]);
					}
				}
			...
			$config = $this->conf;
			$config_list = str_replace($this->params["replace_path"], $this->params["replace_uri"], $this->conf["NAS"]["smb_shares"]);
		}
	...
	private function details(){
		...
		$stats = stat($file);
			...
			$file_["stats"]["time"]["access"] = date("d/m/Y H:i:s", $stats["atime"]);
			$file_["stats"]["time"]["modify"] = @date("d/m/Y H:i:s", $stats["mtime"]);
			$file_["stats"]["time"]["creation"] = @date("d/m/Y H:i:s", $stats["ctime"]);
			$file_["stats"]["permission"]["human"] = $str;
			$file_["stats"]["permission"]["octal"] = sprintf("0%o", 0777 & $p);
			$file_["stats"]["file"]["size"] = $this->_format_bytes($stats["size"]);
		switch($type){
			case "video":
				preg_match("/(.*)(\s+\[(\d)-(\d)\]\s+)\(((\d+)\s+-\s+(.*?))\)(\.\w+)/", $filename, $matched);
				if(count($matched) > 0){
					$file_["type"] = "Film";
					$file_["name"] = trim($matched[1]);
					$file_["part_no"] = trim($matched[3]);
					$file_["tot_pats"] = trim($matched[4]);
					$file_["year"] = $matched[6];
					$file_["director"] = $matched[7];
				} else {
					preg_match("/(.*)\(((\d+)\s+-\s+(.*?))\)(\.\w+)/", $filename, $matched);
					if(count($matched) > 0){
						$file_["type"] = "Film";
						$file_["name"] = trim($matched[1]);
						$file_["year"] = $matched[3];
						$file_["director"] = $matched[4];
					} else {
						preg_match("/(\d+)(\.|-|\s+-|)\s+(.*)(\.\w+)/", $filename, $matched);
						if(count($matched) > 0){
							$file_["type"] = "Serie TV";
							$file_["episode"] = $matched[1];
							$file_["name"] = $matched[3];
						} else {
							preg_match("/(\d+)(\.|-|\s+-|)\s+(.*)(\.\w+)/", $filename, $matched);
							$file_["type"] = "Video";
							$file_["name"] = $matched[3];
			...
			case "audio":
				preg_match("/(\d+)(\.|-|\s+-|)\s+(.*)(\.\w+)/", $filename, $matched);
				$file_["type"] = "Audio Track";
				$file_["no"] = $matched[1];
				$file_["name"] = $matched[3];
				break;
			case "application":
			default:
				preg_match("/(.*)(\.\w+)/", $filename, $matched);
				$file_["type"] = $mtype;
				$file_["name"] = $matched[1];
				break;
		}
		$response["nresults"] = 1;
		$response["nlabels"] = 1;
		$response["results"] = $file_;
		$response["searchtime"] = $this->end_time();
		return $response;
	}
	private function query(){
		...
		if(isset($this->params["q"]) && trim($this->params["q"]) !== "") {
			...
			switch($this->params["op"]){
				case "exactquery":
					$exactresult = true;
					$scanned_files = shell_exec('grep -i "' . $this->params["q"] . '" ' . $config["NAS"]["listing_file_dir"] . 'listing' . $search_param);
					break;
				case "query":
					$exactresult = false;
					$scanned_files = shell_exec('grep -i "' . $this->params["q"] . '" ' . $config["NAS"]["listing_file_dir"] . 'listing' . $search_param);
